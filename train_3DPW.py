...
def train_step(human_motion_input, human_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    if config.deriv_input:
        ...
        human_motion_input_ = torch.matmul(dct_m[:, :, :config.motion.input_length], human_motion_input_.cuda().float())
    else:
        human_motion_input_ = human_motion_input.clone()
    ...
    if config.use_identity_loss:
        ...
        human_motion_input_reverse_ = torch.matmul(dct_m[:, :, :config.motion.input_length], human_motion_input_reverse.cuda().float())
        #prediction
...
val_dataset = PW3D_Eval(config,split=1, data_aug=False)
val_shuffle = False
val_sampler = None
