...
from datasets_setting.cmu_mocap import Datasets as CMU_Dataset
from datasets_setting.cmu_mocap_eval import Datasets as CMU_Eval
from test_CMU import test
import torch
...
config._curUsed=config.CMU_Mocap
...
def train_step(human_motion_input, human_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    if config.deriv_input:
        b,n,c = human_motion_input.shape
    ...
    motion_pred = model(human_motion_input_.cuda())
    ...
    if config.deriv_output:
        ...
        motion_pred = motion_pred[:, :config.motion.target_length]

    b,n,c = human_motion_target.shape
    motion_pred = motion_pred.reshape(b,n,config._curUsed.node_n,3).reshape(-1,3)
    human_motion_target = human_motion_target.cuda().reshape(b,n,config._curUsed.node_n,3).reshape(-1,3)
    loss = torch.mean(torch.norm(motion_pred - human_motion_target, 2, 1))

    if config.use_identity_loss:
        #receive the reverse input
        human_motion_input_reverse = human_motion_input[:, config.motion.target_length:, ].clone().cuda()
        # motion_pred=motion_pred.reshape(b,n,22,3).reshape(b,n,-1)
        human_motion_input_reverse = torch.cat([human_motion_input_reverse, human_motion_target.reshape(b,config.motion.target_length,-1)], dim=1)
        reverse_input_idx = [i for i in range(human_motion_input_reverse.size(1) - 1, -1, -1)]
        reverse_input_idx = torch.LongTensor(reverse_input_idx)
        human_motion_input_reverse = human_motion_input_reverse[:,reverse_input_idx]
        #convert the reverse input to the DCT-based features
        human_motion_input_reverse_ = torch.matmul(dct_m[:, :, :config.motion.input_length], human_motion_input_reverse.cuda())
        #prediction
        motion_pred_reverse = model(human_motion_input_reverse_.cuda())
        ...
        reverse_offset = human_motion_input_reverse[:, -1:].cuda()
        motion_pred_reverse = motion_pred_reverse[:, :config.motion.target_length] + reverse_offset
        motion_pred_reverse = motion_pred_reverse.reshape(b, n, config._curUsed.node_n, 3).reshape(-1, 3)
        human_motion_target_reverse=human_motion_input[:, :config.motion.target_length, ].clone().cuda()
        reverse_target_idx = [i for i in range(human_motion_target_reverse.size(1) - 1, -1, -1)]
        reverse_target_idx = torch.LongTensor(reverse_target_idx)
        human_motion_input_reverse = human_motion_input_reverse[:,reverse_target_idx]
        human_motion_input_reverse = human_motion_input_reverse.cuda().reshape(b, config.motion.target_length, config._curUsed.node_n, 3).reshape(-1, 3)
        reverse_loss = torch.mean(torch.norm(motion_pred_reverse - human_motion_input_reverse, 2, 1))
        loss=loss+config.reverse_weight*reverse_loss

    if config.use_relative_loss:
        motion_pred = motion_pred.reshape(b,n,config._curUsed.node_n,3)
        dmotion_pred = gen_velocity(motion_pred)
        motion_gt = human_motion_target.reshape(b,n,config._curUsed.node_n,3)
...
#set model
config.motion_mlp.hidden_dim = config._curUsed.node_n*3
config.motion.dim = config._curUsed.node_n*3
model = Model(config)
model.train()
model.cuda()
...
dataset = CMU_Dataset(config,split=0, data_aug=config.data_aug)
shuffle = True
sampler = None
...
eval_dataset = CMU_Eval(config,split=2, data_aug=False)
shuffle = False
sampler = None
